...
static struct json_value_s *json_get(struct json_value_s *value, const char *key) {
...
static unsigned long json_as_uint(struct json_value_s *value) {
    struct json_number_s *number = json_value_as_number(value);
    return strtoul(number->number, NULL, 10);
}

void mem_locs_set(CPU *cpu, struct json_value_s *value) {
    ...
    for (struct json_array_element_s *element = array->start; element != NULL; element = element->next) {
        struct json_array_s *pair = json_value_as_array(element->value);
        uint16_t addr = (uint16_t)json_as_uint(pair->start->value);
        uint8_t val = (uint8_t)json_as_uint(pair->start->next->value);
        cpu->ram[addr] = val;
    }
}

void mem_locs_test(CPU *cpu, struct json_value_s *value) {
    struct json_array_s *array = json_value_as_array(value);
    for (struct json_array_element_s *element = array->start; element != NULL; element = element->next) {
        struct json_array_s *pair = json_value_as_array(element->value);
        uint16_t addr = (uint16_t)json_as_uint(pair->start->value);
        uint8_t expected = (uint8_t)json_as_uint(pair->start->next->value);
        uint8_t byte = cpu->ram[addr];
        TEST_CHECK(byte == expected);
        TEST_MSG("addr=0x%04x expected=0x%02x actual=0x%02x", (unsigned)addr, (unsigned)expected, (unsigned)byte);
    }
}
...
void test_opcode(CPU *cpu, const char *opcode) {
    char path[sizeof("../test/65x02/nes6502/v1/00.json")];
    int path_length = snprintf(path, sizeof(path), "../test/65x02/nes6502/v1/%s.json", opcode);
    TEST_ASSERT(path_length > 0 && (size_t)path_length < sizeof(path));
    ...
    TEST_ASSERT(fread(contents, 1, size, file) == size);
    fclose(file);

    struct json_value_s *root = json_parse(contents, size);
    TEST_ASSERT(root != NULL);

    struct json_array_s *test_cases = json_value_as_array(root);
    for (struct json_array_element_s *elem = test_cases->start; elem != NULL; elem = elem->next) {
        struct json_value_s *test_case = elem->value;

        struct json_string_s *name = json_value_as_string(json_get(test_case, "name"));
        TEST_CASE(name->string);

        struct json_value_s *init = json_get(test_case, "initial");
        ...
        mem_locs_set(cpu, json_get(init, "ram"));

        cpu_execute_instruction(cpu);

        struct json_value_s *final = json_get(test_case, "final");
        ...
        mem_locs_test(cpu, json_get(final, "ram"));
    }
